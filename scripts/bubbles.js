...
const getRandomNumber = (min, max) => Math.ceil(Math.random() * (max - min)) + Math.round(min);

const getRandomBubbleRadius = () => getRandomNumber(10, 20);

const getRandomBubblesCount = () => getRandomNumber(0, 6);

const getRandomBubbleX = (radius) => getRandomNumber(radius * 1.5, document.documentElement.offsetWidth - radius * 1.5);
const getRandomBubbleY = () => getRandomNumber(document.documentElement.clientHeight, 
    document.documentElement.clientHeight);

const getRandomTime = () => getRandomNumber(4, 7);

const createAndAddedBubbles = () => {
    const countBubbles = getRandomBubblesCount();

    for(let i = 0; i < countBubbles; i++) {
        const radius = getRandomBubbleRadius();
        
        const x = getRandomBubbleX(radius);
        const y = getRandomBubbleY();

        const width = radius*2;
        const height = width;

        const bubble = createBubble(x, y, width, height);

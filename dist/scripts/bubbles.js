...
var createBubble = function createBubble(x, y, width, height) {
...
var getRandomNumber = function getRandomNumber(min, max) {
  return Math.ceil(Math.random() * (max - min)) + Math.round(min);
};

var getRandomBubbleRadius = function getRandomBubbleRadius() {
  return getRandomNumber(10, 20);
};

var getRandomBubblesCount = function getRandomBubblesCount() {
  return getRandomNumber(0, 6);
};

var getRandomBubbleX = function getRandomBubbleX(radius) {
  return getRandomNumber(radius * 1.5, document.documentElement.offsetWidth - radius * 1.5);
};

var getRandomBubbleY = function getRandomBubbleY() {
  return getRandomNumber(document.documentElement.clientHeight, document.documentElement.clientHeight);
};

var getRandomTime = function getRandomTime() {
  return getRandomNumber(4, 7);
};

var createAndAddedBubbles = function createAndAddedBubbles() {
  var countBubbles = getRandomBubblesCount();

  var _loop = function _loop(i) {
    var radius = getRandomBubbleRadius();
    var x = getRandomBubbleX(radius);
    var y = getRandomBubbleY();
    var width = radius * 2;
    var height = width;
